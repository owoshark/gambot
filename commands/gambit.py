...
def profit(num):
        ...
        for bet in response2.json()['item']['bet_types_matches']:
            if bet['bet_type']['label'] == 'Pick the Winner':
                ...
                if profit > max_profit:
                    ...
                    max_profit = profit
                    profit_id = id
                    profit_name = response2.json()['item']['name']  + " " + datetime.fromisoformat(response2.json()['item']['datetime'].replace("Z", "")).strftime("%x")
                    profit_odds1 = odds1
                    profit_odds2 = odds2
                    bet1 = x
                    bet2 = y
    ...
    embed.add_field(name='Tokens', value=tokens)
    embed.set_footer(text='Swagit donations accepted @')
